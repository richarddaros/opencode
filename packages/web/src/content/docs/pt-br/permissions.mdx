...
- `destructive_bash` — confirmação única para comandos de shell destrutivos ou sensíveis, pedida além da regra normal de `bash`
...
Edições, testes, builds e inspeção de Git no projeto seguem automáticos. Operações destrutivas de shell (`rm`, `mv`, `cp` com potencial de sobrescrita, operações destrutivas de Git, reescritas in-place, ferramentas de disco, escrita no banco e publicação) usam `destructive_bash`, sempre com aprovação única e sem criar regra `always`. O comando continua sendo avaliado também pela regra normal de `bash`, de modo que ser destrutivo nunca o faz escapar de um `bash: "ask"` configurado. A rede permanece desabilitada no container; use uma integração revisada explicitamente quando a operação precisar alcançar um serviço externo.
