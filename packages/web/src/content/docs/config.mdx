...
The built-in `command-validator` and `session-summarizer` agents — used by the [auto agent](/docs/agents#use-auto) to approve permission asks with an LLM — accept the same options. Point them at a cheaper or local model, or override their prompts:
...
```json title="opencode.json"
{
  ...
  "agent": {
    "command-validator": {
      ...
      "prompt": "{file:./prompts/command-validator.txt}"
    },
    "session-summarizer": {
      "model": "ollama/qwen3:4b-instruct",
      "prompt": "{file:./prompts/session-summarizer.txt}"
    }
  }
}
```

With no config, both default to the session provider's small model. The `docs/auto-mode.md` design record in the repository covers the full flow, provider examples (OpenRouter, Ollama), and the audit trail.
